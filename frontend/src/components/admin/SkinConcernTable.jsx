...
export default function SkinConcernTable() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (res.success) setConcerns(res.data);
    } catch {
      setErrorMsg("Gagal memuat data masalah kulit.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  // Handlers
  const handleOpenForm = (concern = null) => {
    setErrorMsg("");
    if (concern) {
      setSelectedConcern(concern);
      setNama(concern.nama);
    } else {
  ...
  const handleSave = async () => {
    if (!nama.trim()) {
      setErrorMsg("Nama Masalah Kulit wajib diisi");
      return;
    }
    try {
      ...
      if (selectedConcern) {
        await updateConcern(selectedConcern.id, payload);
      } else {
        await addConcern(payload);
      }
  ...
  const handleDelete = async () => {
    try {
      setIsSubmitting(true);
      await deleteConcern(selectedConcern.id);
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex justify-between items-center mb-4">
        <h2 className="font-admin-headline-md text-[18px]">
          Data Masalah Kulit (Skin Concern)
        </h2>
        <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          >
            <h2 className="font-admin-headline-md mb-4">
              {selectedConcern ? "Edit Skin Concern" : "Add New Skin Concern"}
            </h2>
            {errorMsg && (
            ...
            <div className="flex flex-col gap-4 mb-6">
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  placeholder="e.g. Acne, Dark Spot, Wrinkles"
                />
              </div>
            </div>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          >
            ...
            <h3 className="font-admin-headline-md mb-2">
              Hapus Masalah Kulit?
            </h3>
            <p className="font-admin-body-md text-[13px] mb-5 text-gray-500">
              Data <strong>{selectedConcern?.nama}</strong> akan dihapus
              permanen.
            </p>

...
export default function SkinTypeTable() {
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const res = await getSkinTypes();
      if (res.success) setSkinTypes(res.data);
    } catch {
      setErrorMsg("Gagal memuat data tipe kulit.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    if (!nama.trim()) {
      setErrorMsg("Nama Tipe Kulit wajib diisi");
      return;
    }
    try {
      ...
      if (selectedType) {
        await updateSkinType(selectedType.id, payload);
      } else {
        await addSkinType(payload);
      }
  ...
  const handleDelete = async () => {
    try {
      setIsSubmitting(true);
      await deleteSkinType(selectedType.id);
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex justify-between items-center mb-4">
        <h2 className="font-admin-headline-md text-[18px]">Data Tipe Kulit</h2>
        <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          >
            <h2 className="font-admin-headline-md mb-4">
              {selectedType ? "Edit Skin Type" : "Add New Skin Type"}
            </h2>
            {errorMsg && (
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          >
            ...
            <h3 className="font-admin-headline-md mb-2">Hapus Tipe Kulit?</h3>
